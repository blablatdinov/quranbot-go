...
func (s *BotService) GetOrCreateSubscriber(chatId int64) (string, error) {
	...
	if err == nil {
		if subscriber.IsActive == true {
			...
			if err := s.RegisterSubscriber(chatId); err != nil {
				return "", err
			}
			return "register", nil
		}
...
func (s *BotService) RegisterSubscriber(chatId int64) error {
	return s.repo.CreateSubscriber(chatId)
}
